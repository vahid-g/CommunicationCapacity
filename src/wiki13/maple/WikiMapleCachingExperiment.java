...
public class WikiMapleCachingExperiment {
	...
	try {
	    ...
	    if (cl.hasOption("query") || cl.hasOption("timing") || cl.hasOption("single")) {
		...
		if (cl.hasOption("query")) {
			...
			if (cl.hasOption("boost")) {
			    results = WikiExperiment.runQueriesOnGlobalIndex(indexPath, queries, gamma, true);
			} else {
			    results = WikiExperiment.runQueriesOnGlobalIndex(indexPath, queries, gamma);
			}
			...
			for (int expNo = 1; expNo <= partitionCount; expNo++) {
			    String indexPath = indexDirPath + expNo;
			    long startTime = System.currentTimeMillis();
			    WikiExperiment.runQueriesOnGlobalIndex(indexPath, queries, gamma);
			    long spentTime = System.currentTimeMillis() - startTime;
			    times[expNo - 1] += spentTime;
			    LOGGER.log(Level.INFO, "Time spent on querying " + queries.size() + " queries is "
				    + spentTime + " seconds");
			}
		    }
