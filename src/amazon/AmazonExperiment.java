...
public class AmazonExperiment {
	...
	public AmazonExperiment(int experimentNumber, int partitionCount, String experimentName, String isbnsFilePath) {
		this.expNo = experimentNumber;
		this.total = partitionCount;
		this.experimentName = experimentName;
		indexPath = AmazonDirectoryInfo.GLOBAL_INDEX_DIR + experimentName + "/" + expNo;
	...
	public static void main(String[] args) {
		...
		AmazonExperiment experiment = new AmazonExperiment(expNo, totalPartitionNo, experimentName, amazonPathFile);
	...
	void expOnGlobalIndex(Map<String, Float> fieldToBoost, String queryFile, String qrelFile, String[] queryFields,
		...
		Map<String, String> isbnToLtid = AmazonIsbnConverter.loadIsbnToLtidMap(AmazonDirectoryInfo.ISBN_DICT);
		for (QueryResult qr : results) {
			AmazonQueryResultProcessor.convertIsbnAnswersToLtidAndFilter(qr, isbnToLtid);
		}
		LOGGER.log(Level.INFO, "Writing results to file..");
		File resultDir = new File(
				AmazonDirectoryInfo.RESULT_DIR + "amazon_f" + fields.length + "_" + this.experimentName);
		resultDir.mkdirs();
		String resultPath = resultDir.getAbsolutePath() + "/" + expNo + ".csv";
		if (extraLogging) {
			Map<String, Set<String>> ltidToIsbns = AmazonIsbnConverter.loadLtidToIsbnMap(AmazonDirectoryInfo.ISBN_DICT);
			AmazonIsbnPopularityMap aipm = AmazonIsbnPopularityMap.getInstance(isbnsFilePath);
			String logPath = AmazonDirectoryInfo.RESULT_DIR + this.experimentName + "_" + expNo + ".log";
			try (FileWriter fw = new FileWriter(resultPath);
				...
				for (QueryResult mqr : results) {
					...
					fw2.write(AmazonQueryResultProcessor.generateLog(mqr, ltidToIsbns, aipm, reader) + "\n");
				}
			} catch (IOException e) {
