...
public class AmazonMapleExperiment {
	...
	private static final Logger LOGGER = Logger.getLogger(AmazonMapleExperiment.class.getName());
	private static final String DATA_FOLDER = "/data/ghadakcv/data/";
	private static final String FILE_LIST = DATA_FOLDER + "amazon_path_integ.csv";
	private static final String ISBN_DICT_PATH = DATA_FOLDER + "amazon-lt.isbn.thingID";
	...
	private static final AmazonDocumentField[] fields = { AmazonDocumentField.TITLE, AmazonDocumentField.CONTENT,
			AmazonDocumentField.CREATORS, AmazonDocumentField.TAGS, AmazonDocumentField.DEWEY };
	private static final String[] queryFields = { "mediated_query", "title", "group", "narrative" };
	...
	public static void main(String[] args) throws IOException {
		...
		List<ExperimentQuery> queries = QueryServices.loadInexQueries(QUERY_FILE, QREL_FILE, queryFields);
		...
		Map<String, String> isbnToLtid = AmazonIsbnConverter.loadIsbnToLtidMap(ISBN_DICT_PATH);
		for (int i = 1; i <= 50; i++) {
			List<QueryResult> results = QueryServices.runQueriesWithBoosting(queries, INDEX_PATH, new BM25Similarity(),
					fieldBoostMap, 200);
			int subsetSize = (int) (i * sortedLtidList.size() / 50.0);
			TreeSet<String> cache = new TreeSet<String>(sortedLtidList.subList(0, subsetSize));
			for (QueryResult result : results) {
				filterCacheResults(result, cache);
				AmazonQueryResultProcessor.convertIsbnAnswersToLtidAndFilter(result, isbnToLtid);
	...
	static List<String> loadIsbnList(String isbnListPath) throws IOException {
		...
		for (String line : Files.readAllLines(Paths.get(isbnListPath))) {
			if (line.contains("/") && line.contains(".")) {
				String isbn = line.substring(line.lastIndexOf('/') + 1, line.indexOf('.'));
