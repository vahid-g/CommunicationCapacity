...
public class AmazonPopularityMiner {
	...
	public static void main(String[] args) throws IOException {
		...
		try (FileWriter fw = new FileWriter("amazon_books_metadata.csv")) {
			for (String isbn : Files.readAllLines(Paths.get(isbnsFile))) {
				...
				String rank = extractFieldFromAmazonHtml(content, "#([0-9,]+) in Books");
				System.out.println(rank);
				String revCount = extractFieldFromAmazonHtml(content, "([0-9,]+) costumer reviews?");
				String rev = extractFieldFromAmazonHtml(content, "([0-9.]+) out of 5 stars");				
	...
	private static String extractFieldFromAmazonHtml(String content, String regexPattern) {
