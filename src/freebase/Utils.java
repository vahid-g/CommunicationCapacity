...
public class Utils {
	...
	public static List<FreebaseQuery> flattenFreebaseQueries(List<FreebaseQuery> queries) {
	...
	static void sshConnect() {
		...
		try {
			...
			FreebaseDataManager.session.setConfig("StrictHostKeyChecking", "no");
			...
			int assinged_port = FreebaseDataManager.session.setPortForwardingL(localPort, remoteHost, remotePort);
			FreebaseDataManager.LOGGER.log(Level.INFO,
					"localhost:" + assinged_port + " -> " + remoteHost + ":" + remotePort);
