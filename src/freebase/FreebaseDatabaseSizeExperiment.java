...
public class FreebaseDatabaseSizeExperiment {
	...
	public static final class DocumentFreqComparator implements Comparator<Document> {
		@Override
	...
	public static List<FreebaseQueryResult> singleTable() {
		List<FreebaseQuery> queries = FreebaseDataManager.loadMsnQueries("query_all");
		String indexPath = INDEX_BASE;
		String dataQuery = FreebaseDataManager.buildDataQuery(TBL_NAME, ATTRIBS);
		List<Document> docs = FreebaseDataManager.loadTuplesToDocuments(dataQuery, ATTRIBS,
				FreebaseDataManager.MAX_FETCH);
	...
	/**
	 * Load tuple weights from the query log and runs the queries on a partition of
	 * the table. Size of partition is decided based on config.
	 * 
	 * @param config
	...
	/**
	 * Estimates tuples wieghts based on train query set, creates index and runs the
	 * test queries over this index
	 * 
	 * @param config
	 * @param trainQueries
	 * @param testQueries
	 * @return
	 */
	public static List<FreebaseQueryResult> weightedTablePartition(List<FreebaseQuery> trainQueries,
			List<FreebaseQuery> testQueries, int partitionSize) {
		LOGGER.log(Level.INFO, "Loading tuples..");
		String dataQuery = FreebaseDataManager.buildDataQuery(TBL_NAME, ATTRIBS);
		TreeMap<String, Float> weights = FreebaseDataManager.loadQueryWeights(trainQueries);
		List<Document> docs = FreebaseDataManager.loadTuplesToDocumentsSmoothing(dataQuery, ATTRIBS,
		...
		FreebaseDataManager.createIndex(docs, (int) (partitionSize * docs.size()), ATTRIBS, indexPaths);
	 ...
	 */
	public static Map<FreebaseQuery, List<FreebaseQueryResult>> databaseSizeOptimal(int partitionCount) {
		LOGGER.log(Level.INFO, "Loading queries..");
		List<FreebaseQuery> queryList = FreebaseDataManager.loadMsnQueriesWithMaxMrr(queryTableName, 1);
		TreeMap<String, Float> weights = FreebaseDataManager.loadQueryWeights(queryList);
		String dataQuery = FreebaseDataManager.buildDataQuery(TBL_NAME, ATTRIBS);
		List<Document> docs = FreebaseDataManager.loadTuplesToDocumentsSmoothing(dataQuery, ATTRIBS,
				FreebaseDataManager.MAX_FETCH, weights, 1.0, TBL_SIZE);
		// Collections.shuffle(docs);
		Collections.sort(docs, new DocumentFreqComparator());
		LOGGER.log(Level.INFO, "max weight: ${0}", docs.get(0).get(FreebaseDataManager.FREQ_ATTRIB));
		...
		for (int i = 0; i < partitionCount; i++) {
			...
			FreebaseDataManager.createIndex(docs, (int) (((i + 1.0) / partitionCount) * docs.size()), ATTRIBS,
					indexPaths[i]);
			LOGGER.log(Level.INFO, "Submitting queries..");
			List<FreebaseQueryResult> resultList = FreebaseDataManager.runFreebaseQueries(queryList, indexPaths[i]);
	 ...
	 * @return a list of FreebaseQueryResults objects.
	 */
	public static Map<FreebaseQuery, List<FreebaseQueryResult>> databaseSizeOptimalFlat(int partitionCount) {
		LOGGER.log(Level.INFO, "Loading queries..");
		List<FreebaseQuery> queryList = Utils
				.flattenFreebaseQueries(FreebaseDataManager.loadMsnQueriesWithMaxMrr(queryTableName, 1));
		TreeMap<String, Float> weights = FreebaseDataManager.loadQueryInstanceWeights(queryList);
		String dataQuery = FreebaseDataManager.buildDataQuery(TBL_NAME, ATTRIBS);
		List<Document> docs = FreebaseDataManager.loadTuplesToDocumentsSmoothing(dataQuery, ATTRIBS,
				FreebaseDataManager.MAX_FETCH, weights, 1.0, TBL_SIZE);
		// Collections.shuffle(docs);
	 ...
	 * @return
	 */
	public static Map<FreebaseQuery, List<FreebaseQueryResult>> databaseSize(int partitionCount, double trainSize) {
		LOGGER.log(Level.INFO, "Loading queries..");
		List<FreebaseQuery> queryList = FreebaseDataManager.loadMsnQueriesWithMaxMrr(queryTableName, 1);
		List<FreebaseQuery> flatQueryList = Utils.flattenFreebaseQueries(queryList);
		// random sampling
		Collections.shuffle(flatQueryList);
		List<FreebaseQuery> trainQueries = flatQueryList.subList(0, (int) (flatQueryList.size() * trainSize));
		...
		LOGGER.log(Level.INFO, "Loading tuples..");
		String dataQuery = FreebaseDataManager.buildDataQuery(TBL_NAME, ATTRIBS);
		TreeMap<String, Float> weights = FreebaseDataManager.loadQueryInstanceWeights(trainQueries);
		List<Document> docs = FreebaseDataManager.loadTuplesToDocumentsSmoothing(dataQuery, ATTRIBS,
				FreebaseDataManager.MAX_FETCH, weights, 1, TBL_SIZE);
		Collections.shuffle(docs);
		Collections.sort(docs, new DocumentFreqComparator());
	...
	static void writeFreebaseQueryResults(Map<FreebaseQuery, List<FreebaseQueryResult>> fqrMap, String resultFileName) {
		...
		try {
			...
			for (FreebaseQuery query : fqrMap.keySet()) {
				fw.write(query.id + ", " + query.text + ", " + query.frequency + ", " + query.instanceId);
