...
public class FreebaseDatabaseSizeExperiment {
	...
	public static void singleTable(String tableName) {
		...
		String dataQuery = FreebaseDataManager.buildDataQuery(tableName,
				attribs);
		FreebaseDataManager.createIndex(FreebaseDataManager
				.loadTuplesToDocumentArray(dataQuery, attribs), attribs,
				indexPath);
	...
	public static void databaseSizeStratified(String tableName,
		...
		for (int i = 0; i < PARTITION_COUNT; i++) {
			...
			FreebaseDataManager.createIndex(nonRels,
					(int) (nonRels.size() * (i + 1.0) / PARTITION_COUNT), attribs,
					indexPaths[i]);
			FreebaseDataManager.appendIndex(rels,
					(int) (rels.size() * (i + 1.0) / PARTITION_COUNT), attribs,
					indexPaths[i]);
		}
		LOGGER.log(Level.INFO, "Submitting Queries..");
	...
	public static void randomizedDatabaseSize(String tableName, int experimentNo) {
		...
		String indexQuery = FreebaseDataManager.buildDataQuery(tableName,
				attribs);
		Document[] docs = FreebaseDataManager.loadTuplesToDocumentArray(
				indexQuery, attribs);
