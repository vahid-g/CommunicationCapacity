...
public class FreebaseSchemaCapacityExperiment {
	...
	try (FileWriter fw = new FileWriter(FreebaseDirectoryInfo.RESULT_DIR
		+ "t-" + tableName + " q-" + tableName + " a-name" + ".csv");) {
	    for (FreebaseQuery query : queries) {
		FreebaseQueryResult fqr = FreebaseDataManager.runFreebaseQuery(
			query, indexPath);
		fw.write(query.id + ", " + query.text + ", " + query.wiki
			+ ", " + fqr.p3() + ", " + fqr.precisionAtK(10) + ", "
			+ fqr.mrr() + "\n");
	    }
	} catch (Exception e) {
	    e.printStackTrace();
