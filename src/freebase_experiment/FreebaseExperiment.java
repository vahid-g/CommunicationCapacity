...
public class FreebaseExperiment {
	...
	public static void experiment_singleTable(String tableName, String[] attribs) {
		...
		FileWriter fw = null;
		try {
			fw = new FileWriter(FreebaseDataManager.resultDir + tableName + ".csv");
	...
	public static void runExperiment2() {
		...
		for (FreebaseQuery query : queries) {
			...
			query.attribs.put(FreebaseDataManager.NAME_ATTRIB,
					query.text.toLowerCase().replace(keyword, ""));
			query.attribs.put(FreebaseDataManager.DESC_ATTRIB,
					query.text.toLowerCase().replace(keyword, ""));
		}
		try (FileWriter fw = new FileWriter(FreebaseDataManager.resultDir + tableName
	...
	public static void runExperiment3() {
		...
		String attribs[] = { FreebaseDataManager.NAME_ATTRIB, FreebaseDataManager.DESC_ATTRIB, FreebaseDataManager.SEMANTIC_TYPE_ATTRIB };
		String indexPath = FreebaseDataManager.INDEX_BASE + tableName + "/";
		// createIndex(tableName, attribs, indexPath);
		String pattern = "program| tv| television| serie| show | show$| film | film$| movie";
		String sql = "select * from query where text REGEXP '" + pattern
	...
	public static void experiment_keywordExtraction(String tableName,
		...
		try (FileWriter fw = new FileWriter(FreebaseDataManager.resultDir + "t-" + tableName
	...
	public static void experiment_databaseSize() {
		...
		for (int i = 0; i < 10; i++) {
			...
			indexPaths[i] = FreebaseDataManager.INDEX_BASE + tableName + "_" + i + "/";
		}
		System.out.println("submitting queries..");
		FileWriter fw = null;
		try {
			fw = new FileWriter(FreebaseDataManager.resultDir + tableName + "_mrr.csv");
			for (FreebaseQuery query : queries) {
				fw.write(query.id + ", " + query.text + ", " + query.frequency
						+ ", ");
				for (int i = 0; i < 10; i++) {
					FreebaseQueryResult fqr = FreebaseDataManager.runFreebaseQuery(query,
							indexPaths[i]);
					fw.write(fqr.mrr() + ", ");
				}
				fw.write("\n");
	...
	public static void experiment_databaseSizeRandomized(int experimentNo) {
		...
		FileWriter fw = null;
		try {
			fw = new FileWriter(FreebaseDataManager.resultDir + tableName + "_randomized_p3_"
					+ experimentNo + ".csv");
			for (FreebaseQuery query : queries) {
	...
	public static ExperimentResult experiment_databaseSizeDoubleRandomized(
		...
		for (int i = 0; i < queryPartitionCount; i++) {
			...
			for (int j = 0; j < dbPartitionCounts; j++) {
				List<FreebaseQueryResult> queryResults = FreebaseDataManager.runFreebaseQueries(
	...
	public static void experiment_repeatRandomizedDatabaseSize() {
		...
		FileWriter fw = null;
		try {
			fw = new FileWriter(FreebaseDataManager.resultDir + "result_lost.csv");
		...
		try {
			fw = new FileWriter(FreebaseDataManager.resultDir + "result_found.csv");
			for (int i = 0; i < qCount; i++) {
				for (int j = 0; j < dCount - 1; j++) {
	...
	public static void main(String[] args) {
		 ...
		 */
		experiment_repeatRandomizedDatabaseSize();
