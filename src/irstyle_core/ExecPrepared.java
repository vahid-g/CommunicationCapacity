...
private ArrayList[] resultSets; // list of ResultSet lists. One for each CN
  ...
  private int getResults(JDBCaccess jdbcacc,PreparedStatement prepared,ArrayList[] S,int[] indexToBeChecked)
  {//obsolete
    try{
    ...
    {
      System.out.println("exception class: " + e1.getClass() +"  with message: " + e1.getMessage()+" exception in  ExecPrepared.getResults");
      e1.printStackTrace() ;
      return -1;
    }
 ...
 private int getResultsParallel(JDBCaccess jdbcacc,PreparedStatement prepared,ArrayList[] S,int[] indexToBeChecked, int numnfreeTSs)
 {//differs in taht it inputs numnfreeTSs
   //obsolete
   try{
  ...
  //ignore combinations with same tuple for the same tuple set
    for(int k=0;k<nfreeTSs.size();k++)
      for(int l=0;l<nfreeTSs.size();l++)
        if( k!=l && ((String) nfreeTSs.get(k)).compareTo((String) nfreeTSs.get(l))==0 && ((Integer) S[k].get(indexToBeChecked[k])).intValue()  == ((Integer) S[l].get(indexToBeChecked[l])).intValue() )
                    return true;
    return false;
  }
  ...
  private double getScore(double[] scores, int CNSize)
  {//applies ranking function to array of attributes with scores in scores. We just sum them for now.
  double score=0;
  ...
  private double getScore(double[] scores, int CNSize, int numnfreeTSs)
  {//applies ranking function to array of attributes with scores in scores. We just sum them for now.
  double score=0;
  for(int i=0;i<numnfreeTSs;i++)
    score+= scores[i];
  ...
  {//adds result to results array R if result is in top-N results
  ...
  private int check4Results(JDBCaccess jdbcacc,PreparedStatement prepared,ArrayList[] S, ArrayList[] scoresS, int[] indexToBeChecked,ArrayList R, ArrayList scoresR, int N, int CNsize,ArrayList keywords, boolean allKeywInResults)
  { //check for results and add them to R
    //returns new resultsSoFar
    try{
    int numparams=indexToBeChecked.length;
    //int numresults=0;
    for(int i=0;i<numparams;i++)
    {
      int id=((Integer) ((ArrayList) S[i]).get(indexToBeChecked[i])).intValue();
      prepared.setInt(i+1, id);
      if(Flags.DEBUG_INFO2) System.out.print(id+" ");
    }
    if(Flags.DEBUG_INFO2) System.out.println("");
    ResultSet rs=jdbcacc.executePrepared(prepared);
    if(rs==null)
    ...
    {
      System.out.println("exception class: " + e1.getClass() +"  with message: " + e1.getMessage()+" exception in  ExecPrepared.check4Results");
      e1.printStackTrace() ;
      return -1;
    }
  }

  private int check4ResultsParallel(JDBCaccess jdbcacc,PreparedStatement prepared,ArrayList[] S, ArrayList[] scoresS, int[] indexToBeChecked,ArrayList R, ArrayList scoresR, int N, int CNsize, int numnfreeTSs,ArrayList keywords,boolean allKeywInResults)
  { //check for results and add them to R
    //returns new resultsSoFar
  ...
  private double getMax_rTi(int[] B,int[] lookaheadscores, int CNsize)
  {
    ...
    for(int i=0;i<B.length;i++)
    {
      ...
      {
        if(i!=j) scores[j]=B[j];
        else if (lookaheadscores[j]==-1) scores[j]=0;
        else scores[j]=lookaheadscores[j];
      }
  ...
  boolean foundTopNParallel(ArrayList R,ArrayList scoresR,int[][] lookaheadscores,int[][] B, int N,int[] CNsize, int[] numnfreeTSs)
  {
   //check if R.size<N or R[N]<max r(Ti)
     if(R.size()>N)
       System.out.println("Error: R.size()>N");
     ...
     else
     {
       ...
       for(int c=0;c<numCNs;c++)
       {
         ...
         if(max_rTiofCN>max_rTi)
           max_rTi=max_rTiofCN;
         if(Flags.DEBUG_INFO2) System.out.println("CN:"+c+"max_rTiofCN="+max_rTiofCN);
       }
  ...
  public int ExecuteParameterized(JDBCaccess jdbcacc, String sql, ArrayList nfreeTSs, ArrayList keywords, int N, int CNsize, ArrayList ResultsAndScores,boolean allKeywInResults)
  {//returns time
    //also adds results and their scores to ResultsAndScores, which is an ArrayList of type Result
    ...
    ArrayList[] scoresS=new ArrayList[numnfreeTSs]; //keep the scores of the ids retrieved so far from nfree TS[i]
    ...
    int[] indexToBeChecked=new int[numnfreeTSs]; //used to select a particular combination of ids to check for a result
    int[] lookahead=new int[numnfreeTSs]; //used to keep ids retrieved but not yet processed. If -1 then no lookahead available
    int[] lookaheadscores=new int[numnfreeTSs]; //used to keep scores of corresponding ids in lookahead. If -1 then no lookahead available
    ...
    if(!Flags.ALLOW_DUPLICATE_TUPLES)
    {
      if(!sameTuple(nfreeTSs,indexToBeChecked,S))
        resultsSoFar=check4Results(jdbcacc,prepared, S, scoresS,indexToBeChecked, R, scoresR,N,CNsize,keywords,allKeywInResults);
    }
    else resultsSoFar=check4Results(jdbcacc,prepared, S, scoresS,indexToBeChecked, R, scoresR,N,CNsize,keywords,allKeywInResults);
    ...
    //check if R.size<N or R[N]<max r(Ti)
    boolean foundtopn=false;//foundTopN(R,scoresR,lookaheadscores,B,N,CNsize); //lookaheadscores[i]=-1 at this point
    while(!foundtopn)
    {
     ...
     //and also get TS with max score
      ...
      for(int i=0;i<numnfreeTSs;i++)
      {
        if(lookaheadscores[i]<0) //no lookahead
        {
          ...
          if(lookahead[i]<0) lookaheadscores[i]=-1;
          if(lookahead[i]>-1)
          {
     ...
     if(allTSsFinished)
           break;
     //replace S[indexOfTopScore] by lookahead[indexOfTopScore] (=topScore). Replace it back later
   ...
   System.out.println("results output = "+resultsSoFar+" numPreparedQueries = "+numPreparedQueries+" in time = "+(time2-time1));
   if(Flags.RESULTS__SHOW_OUTPUT)
 ...
 public int ExecuteParallel(JDBCaccess jdbcacc, String[] sqls, ArrayList[] nfreeTSs, ArrayList keywords, int N, int[] CNsize,boolean allKeywInResults)
 {//input:
  ...
  // nfreeTSs[i]: list of non free TS names for i-th CN
   ...
   for(int i=0;i<numCNs;i++)
     if(nfreeTSs.length>maxnfreeTSsize) maxnfreeTSsize= nfreeTSs.length;
   ArrayList[][] S=new ArrayList[numCNs][maxnfreeTSsize]; //array (for each CN) of arrays (for each nfreeTS, size of max chosen for width, so some useless i,j combinations in S) of Arraylists (of tuple ids)
   ArrayList[][] scoresS=new ArrayList[numCNs][maxnfreeTSsize]; //array (for each CN) of arrays (for each nfreeTS, size of max chosen for width, so some useless i,j combinations in S) of Arraylists (of tuple ids)
   //ArrayList[] S=new ArrayList[numnfreeTSs]; //keep the ids retrieved so far from nfree TS[i]
   //ResultSet[] rs=new ResultSet[numnfreeTSs];
   //int[] topscores=new int[numnfreeTSs]; //max score so far for nfree TS[i]. Score is score(1)+...+score(numkeywords) as defined in MIndexAccess
   int[][] indexToBeChecked=new int[numCNs][maxnfreeTSsize]; //used to select a particular combination of ids to check for a result
   int[][] lookahead=new int[numCNs][maxnfreeTSsize]; //used to keep ids retrieved but not yet processed. If -1 then no lookahead available
   int[][] lookaheadscores=new int[numCNs][maxnfreeTSsize]; //used to keep scores of corresponding ids in lookahead. If -1 then no lookahead available
   int[][] B=new int[numCNs][maxnfreeTSsize]; //score of top tuple for nfree TS[i]. Used instead of the B(TSi)'s in alg.
   ArrayList R=new ArrayList(N); //to store the Strings of top-N results
   ArrayList scoresR=new ArrayList(N); //to store the scores (double) of the top-N results
   ...
   for(int c=0;c<numCNs;c++)
    if(!Flags.ALLOW_DUPLICATE_TUPLES)
    {
     if(!sameTuple(nfreeTSs[c],indexToBeChecked[c],S[c]))
       resultsSoFar=check4ResultsParallel(jdbcacc,prepared[c], S[c], scoresS[c],indexToBeChecked[c], R, scoresR,N,CNsize[c],numnfreeTSs[c],keywords,allKeywInResults);
    }
    else resultsSoFar=check4ResultsParallel(jdbcacc,prepared[c], S[c], scoresS[c],indexToBeChecked[c], R, scoresR,N,CNsize[c],numnfreeTSs[c],keywords,allKeywInResults);
       //resultsSoFar+=getResultsParallel(jdbcacc,prepared[c], S[c], indexToBeChecked[c],numnfreeTSs[c]);
   for(int c=0;c<numCNs;c++)
   for(int i=0;i<numnfreeTSs[c];i++)
   {
    lookahead[c][i]=-1;
   ...
   boolean foundtopn=false;//=foundTopNParallel(R,scoresR,lookaheadscores,B,N,CNsize,numnfreeTSs); //lookaheadscores[i]=-1 at this point
   if(Flags.DEBUG_INFO2) printResults(R,scoresR);
   while(!foundtopn)
   {
    //get CN with max score
     ...
     for(int c=0;c<numCNs;c++)
     {
      for(int i=0;i<numnfreeTSs[c];i++)
      {
       ...
       {
        lookahead[c][i]=jdbcacc.getNextID((ResultSet) resultSets[c].get(i)); //if finished will return -1
        if(lookahead[c][i]<0) lookaheadscores[c][i]=-1;
        if(lookahead[c][i]>-1)
        {
   ...
   //      System.out.println(str);
    ...
    // }
     ...
     //get a tuple from all TSs with no lookahead (anyway, all have due to previous step)
    //and also get TS with max score
    int topScore=-1;
    ...
    for(int i=0;i<numnfreeTSs[CNindexOfTopScore];i++)
    {
     ...
     {
      lookahead[CNindexOfTopScore][i]=jdbcacc.getNextID((ResultSet) resultSets[CNindexOfTopScore].get(i)); //if finished will return -1
      if(lookahead[CNindexOfTopScore][i]>-1)
        lookaheadscores[CNindexOfTopScore][i]=jdbcacc.getCurrScore((ResultSet) resultSets[CNindexOfTopScore].get(i));
      allTSsFinished=false;
     }
    ...
    scoresS[CNindexOfTopScore][indexOfTopScore].add(new Integer(lookaheadscores[CNindexOfTopScore][indexOfTopScore]));
    ...
    for(int j=0;j<numcombinations;j++)
    {
     ...
     indexToBeChecked[CNindexOfTopScore]=getACombination(S[CNindexOfTopScore],j,numnfreeTSs[CNindexOfTopScore]);
     //ignore combinations with same tuple for the same tuple set
    if(!Flags.ALLOW_DUPLICATE_TUPLES)
    {
     if(!sameTuple(nfreeTSs[CNindexOfTopScore],indexToBeChecked[CNindexOfTopScore],S[CNindexOfTopScore]))
     {
       resultsSoFar=check4ResultsParallel(jdbcacc,prepared[CNindexOfTopScore], S[CNindexOfTopScore], scoresS[CNindexOfTopScore],indexToBeChecked[CNindexOfTopScore], R, scoresR,N,CNsize[CNindexOfTopScore],numnfreeTSs[CNindexOfTopScore],keywords,allKeywInResults);
      //resultsSoFar+=getResultsParallel(jdbcacc,prepared[CNindexOfTopScore], S[CNindexOfTopScore], indexToBeChecked[CNindexOfTopScore],numnfreeTSs[CNindexOfTopScore]);
       if(foundtopn=foundTopNParallel(R,scoresR,lookaheadscores,B,N,CNsize,numnfreeTSs))
                 break;
     }
    }
  ...
  long time2=System.currentTimeMillis() ;
  System.out.println("Parallel algor: results output = "+resultsSoFar+" numPreparedQueries = "+numPreparedQueries+ "  in time = "+(time2-time1));
  if(Flags.RESULTS__SHOW_OUTPUT)
     printResults(R,scoresR);
  return (int) (time2-time1);
 }
...
int ExecuteParameterizedSymmetric(JDBCaccess jdbcacc, String sql, ArrayList nfreeTSs, ArrayList keywords, int N, int CNsize, ArrayList ResultsAndScores,boolean allKeywInResults)
{//differs from ExecuteParameterized in that it goes down to TSs in parallel and does not pick  the tuple with maximum r(Ti)
   //returns time
  //also adds results and their scores to ResultsAndScores, which is an ArrayList of type Result
  if(Flags.DEBUG_INFO)
  {
  ...
  while(!foundtopn)
  {
   ...
   //indexOfTopScore=getIndexOfMax_rTi(B,lookaheadscores,CNsize); //in non symmetric
   if(allTSsFinished)
         break;
   if(foundtopn=foundTopN(R,scoresR,lookaheadscores,B,N,CNsize))
     break;
...
public int ExecuteParallelSymmetric(JDBCaccess jdbcacc, String[] sqls, ArrayList[] nfreeTSs, ArrayList keywords, int N, int[] CNsize,boolean allKeywInResults)
 {//input:
  // sqls[i]: param sql for i-th CN
  // nfreeTSs[i]: list of non free TS names for i-th CN
   ...
   for(int l=0;l<numCNs;l++) nextIndex[l]=0;
   //check if R.size<N or R[N]<max u(Ci)
   boolean foundtopn=false;//foundTopNParallel(R,scoresR,lookaheadscores,B,N,CNsize,numnfreeTSs); //lookaheadscores[i]=-1 at this point
   while(!foundtopn)
   {
   ...
   //      System.out.println(str);
    ...
    //indexOfTopScore=getIndexOfMax_rTiParallel(B[CNindexOfTopScore],lookaheadscores[CNindexOfTopScore],CNsize[CNindexOfTopScore],numnfreeTSs[CNindexOfTopScore]); //in non symmetric
    nextIndex[CNindexOfTopScore]= (nextIndex[CNindexOfTopScore]+1)%numnfreeTSs[CNindexOfTopScore];
    indexOfTopScore=getNextIndexSymmetric(lookaheadscores[CNindexOfTopScore],nextIndex[CNindexOfTopScore],numnfreeTSs[CNindexOfTopScore]); //in non symmetric
    //if(allTSsFinished)//removed since allCNsFinished does same job
    //  break;
    //replace S[indexOfTopScore] by lookahead[indexOfTopScore] (=topScore). Replace it back later
    S[CNindexOfTopScore][indexOfTopScore].add(new Integer(lookahead[CNindexOfTopScore][indexOfTopScore]));
